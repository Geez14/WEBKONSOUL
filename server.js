...
const SHELL_ARGS = IS_WINDOWS ? ['-NoProfile', '-Command', '-'] : [];
...
function createTerminalProcess(tabId, clientId) {
    ...
    setTimeout(() => {
        const welcomeMsg = IS_WINDOWS 
            ? `Server Management Console - Tab: ${tabId}\nPowerShell ${process.cwd()}\nPS> `
            : `Server Management Console - Tab: ${tabId}\nBash ${process.cwd()}\n$ `;
...
server.listen(PORT, () => {
    ...
    console.log(`🔧 Shell: ${IS_WINDOWS ? 'PowerShell' : 'Bash'}`);
